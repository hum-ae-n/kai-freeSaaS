...
### First-visit walkthrough (Phase 21)

The first time a device opens the curator (`freestack:v1:curatorcoach` unset), a dismissible panel renders above the link generator: four plain-words steps for someone who has never used it. 1: tick the tools the client needs, or start from a persona pack. 2: add the client's name and an optional personal note. 3: Generate link. 4: send that link, which always points at the public root and never reveals this page. A "How this works" ghost button in the curator header reopens the same panel at any time, so the walkthrough is recoverable rather than once-only; dismissing it writes the key. It is plain flow content above the link generator, not a modal, so no dialog contract applies. Content stays consistent with HOW-TO.md's flow and fits one screen. This is the interface-level answer to the curator being an unlisted page with no login: the URL is the door, the walkthrough is the signage.
...
1. **Hero** (rewritten in Phase 16 on Rocky's direction: "the first section should be distinctive and say what Free Stack is, call it what it is"). It states the proposition, not just the brand:
   ...
   - **Savings ticker (Phase 17, Rocky: "a roller that spins that shows the maximum amount of money you save in pounds and Starbucks coffees").** Three facts, each counting up on arrival: **the starter-stack saving**, **the tool count**, and **paid placements, which is zero**. The saving and the count are computed at runtime from `tools.json`, never hard-coded, exactly as the tool count always was. Paid placements is a literal zero, and should be: there is no field to sum, because the honest expression of "we take no money to list anything" is that the codebase has nowhere to record such a payment. If that ever stops being true the fact has to come down, not be recomputed. The three start one after another, roughly a quarter-second apart, so the eye scans them in order rather than meeting three figures at once.
...
3b. **Why this exists** (Phase 18, Rocky's 11 Aug direction; moved above the shelves in Phase 21, Rocky's 12 Aug direction, keeping the drop-down): one collapsed native `<details>` section between the Discover mount and the shelf band, in the FAQ's visual language, whose `<summary>` row reads "Why this exists" (a real disclosure, 44px minimum target, collapsed by default so the section costs one summary row against the page-height budget; the content is in the DOM whether open or not, which is the point for crawlers). Inside, four short paragraphs: the government's August 2026 crackdown on rip-off pricing and subscription traps with its own figures (about £500 a year per person on subscriptions, £1.6bn a year on unwanted ones), attributed and linked to the Guardian's 9 August 2026 report under the standard external-link rules; that Free Stack was already live before the announcement, built on the same conviction; what the directory actually is (genuinely free tiers with honest limits, at least two alternatives so nobody is locked in, honest value figures, nobody paid to be listed); and what to do with it whether you are a founder starting out or an established business running it as a subscription audit. A closing line links to the fuller story at `/why-register.html` ("Why we built My Stack"), internal link, same tab. The copy is defined once, in `js/why-copy.js`, imported by both `js/public.js` and `scripts/build-seo.mjs` (the `savings-copy.js` precedent), so the rendered section and the static crawler block never drift. The two government figures are the article's, attributed, and must never be recomputed from our data; if the policy or the figures change, the copy changes in `why-copy.js` and nowhere else. No new motion; house style holds (no em dashes, British English).
...
6. **Footer**, a proper sectioned site footer (Phase 19, Rocky's 11 Aug direction: pull the accreted lines out into clear sections and make it a real footer), replacing the seven stacked paragraphs that grew across Phases 11, 13.1 and 15. Public directory only; the client-mode and workspace footers are untouched, and the public footer stops borrowing `.cli-footer`. Structure, on `--paper-2` with a `--bw-heavy` ink top rule, inside the standard content column:
   - **Brand block**: the lockup, "Curated by " with the outbound Kaipability Ltd link, and the no-affiliates practice line.
   - **Explore** (eyebrow heading): **FAQ** (`/faq.html`), **Changelog** (`/changelog.html`), **Privacy** (`/privacy.html`), **Contact** (`/contact.html`).
   - **Also free** (eyebrow heading): **My Stack** (`/my`) with its one-line description, and **Why we built My Stack** (`/why-register.html`).
   - **Work with Kaipability** (eyebrow heading): the "Want a stack chosen for your business? Talk to Kaipability" CTA line, then the payment lines under the existing law, unchanged: `js/payments.js` remains the only place a provider URL may live, an empty URL means the element is never built (not hidden), and the trust sentence renders only when at least one payment link exists.
   - **Bottom line**, full width beneath a hairline rule: the company identity line ("Kaipability Ltd, registered in England and Wales, Company No. 15772934") with the outbound `kaipability.com` and AIRL links, both under the standard external-link rules.
   - At 768px and above the three headed groups sit as columns beside the brand block; below 768px everything stacks, brand block first, bottom line last. Every link keeps the 44px tap floor at touch widths (the 17.5 rule: the padding that buys it is cancelled on both axes where it would open visible gaps). **Every pre-redesign link survives with an identical href and external-link treatment; nothing is added or removed, only regrouped under headings.**
   - The redesign must hold the existing page-height budgets at both reference widths. At 1280px the budget carries single-digit slack, so the columns are expected to pay for themselves against the old stacked paragraphs; if they do not, the answer is tightening the footer, never moving the budget.
...
### First-judgement explainer (Phase 21)

The first time a judgement is recorded on the public surface, whether from the deck or from the browse list's chips and quick-judge buttons, a one-time dialog appears, once per device (`freestack:v1:judgecoach`, a public-surface key outside the `js/my/store.js` seam, same standing as `freestack:v1:discover`). Content, in order, three short beats: judgements are saved **in this browser only**, nothing is sent anywhere and there is no account to create; the subscription-trap line, tying the page to the problem the government's August 2026 crackdown targets (businesses lose track of what they have signed up for, and unwanted subscriptions cost UK consumers about £1.6bn a year); and the pointer to **My Stack**, a free register of what you use and what you plan to try, no account, no password, nothing leaves your device, linking to `/my`. Buttons: a primary "Keep browsing" dismissal and the quiet My Stack link, both 44px minimum. Dialog contract per 16.4: focus moves in on open, Escape and outside-click close, focus returns to the triggering control. It appears instantly with no entrance animation, so the motion inventory is untouched. It never fires during the deck's own first-open coach (if both would trigger, the deck coach wins and the explainer waits for the next judgement), never fires twice, and never appears in client mode or the workspace. Copy lives beside the judgement code, not in `why-copy.js`: this is interface guidance, not page content, and it must not enter the static crawler block.
