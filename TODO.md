...
Build state: Phases 13.1 and 16 merged 3 Aug as `17e7164` via PR 18 on Rocky's word, as one release: the payments footer with the live Stripe tip link, the rewritten hero ("The free software directory for small business") over drifting stack planes, the fixed self-compressing top bar with its burger disclosure, `changelog.html` replacing the homepage's Recently updated strip, and `docs/build-notes/` holding the full session log. Production verified byte-identical. Smoke suite at 431 with five consecutive clean runs; four pre-existing flaky checks were fixed en route, each proven by mutation to still catch its regression.
