 ...
 * persona-aware crawler block), data/category-intros.json and (Phase 16)
 * data/changelog.json, and emits or refreshes:
 ...
 *     JSON-LD)
 *   - changelog.html (Phase 16, static, indexable, the "Recently updated"
 *     strip Rocky asked removed from the homepage, generated from
 *     data/changelog.json the way faq.html is generated from FAQ_ITEMS;
 *     data/changelog.json is itself scripts/changelog.mjs's output, not
 *     this generator's, and is only ever read here)
 *   - sitemap.xml
 *   - llms.txt
 ...
 * tool's own `last_verified` field, from data/changelog.json's own `date`
 * field (changelog.html), or is omitted outright, so running this script
 * twice against the same data/*.json produces byte-identical files. This is
 * what lets CI's drift gate (.github/workflows/ci.yml) simply diff the
 * committed artefacts against a fresh run.
 *
 * Security (PRD section 7, extended to dev-time generation by section 18):
 * every string sourced from data/tools.json, data/category-intros.json or
 * data/changelog.json is untrusted content ultimately written by a
 * non-developer curator, so every one of them is HTML-escaped before being
 * written into a static file. This script is not the el()/textContent
 * runtime discipline (there is no DOM at generation time), but the same
 * threat model applies: an unescaped `<` or `&` in a tool's description
 * would corrupt the page for every visitor, JS or not.
 *
...
/* --- changelog.html (PRD section 16 amended, Phase 16) ----------------------
   The "Recently updated" strip Rocky asked removed from the homepage (2
   Aug: "remove the recently updated and move it") becomes its own static
   page here, generated from data/changelog.json exactly the way faq.html
   is generated from FAQ_ITEMS above: same page mould (head, byte-identical
   theme-boot script so the CSP hash set does not change), indexable,
   canonical, title and meta description, entries as visible static HTML,
   no runtime fetch. data/changelog.json itself is not this generator's
   output: scripts/changelog.mjs derives it separately from tools.json's
   own git history and is run on its own schedule; this function only ever
   reads the committed file, the same trust level buildJsonLdBlock already
   gives active.length elsewhere in this script. Every string is escaped
   (section 7/18 discipline: a tool name reaching this file already passed
   through a human curator, untrusted the same as every other tools.json
   field). */
...
function buildChangelogHtml(entries) {
  const itemsHtml = entries.map((entry) => {
    // Archived entries kept the same "is-archived" marker class the
    // homepage strip gave them and no rule of its own uses it: retirements
    // are a trust signal, not something to visually apologise for (see the
    // PUBLIC block of styles.css's own retired comment on this point), so
    // the class carries over unchanged rather than inventing a new visual
    // treatment the strip never had.
    const cls = entry.kind === 'archived' ? 'changelog-item is-archived' : 'changelog-item';
    return (
      `      <li class="${cls}">\n`
      + `        <span class="changelog-date">${escapeHtml(formatChangelogDate(entry.date))}</span>\n`
      + `        <span class="changelog-detail">${escapeHtml(entry.detail)}</span>\n`
      + '      </li>'
    );
  }).join('\n');
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Changelog · Free Stack by Kaipability</title>
  <meta name="description" content="What has been added, retired or repriced in the Free Stack directory of free software for UK small businesses.">
  ...
  <!-- No-flash-of-wrong-theme boot script, byte-identical to index.html's:
       kept identical on purpose so the CSP script-src hash already
       allow-listed for index.html's boot script covers this page too,
       without a second hash entry in netlify.toml (why-register.html,
       faq.html, privacy.html and contact.html all do the same). Also
       stamps js-boot on the root (Phase 15.4), which only hides anything
       on index.html's own #static-root; harmless here. If this ever needs
       to diverge from index.html's copy, it must get its own hash there. -->
${THEME_BOOT_SCRIPT}
  <link rel="stylesheet" href="css/styles.css">
  ...
  <!-- Page-specific layout only: colours, type and spacing tokens all come
       ...
       needs no hash entry and changes the CSP value not at all. -->
  <style>
    .changelog-wrap { max-width: var(--content-max); margin: 0 auto; padding: var(--s-8) var(--s-6) var(--s-24); }
    .changelog-back { font-size: var(--fs-14); display: inline-block; margin-bottom: var(--s-6); }
    .changelog-header { padding: var(--s-8) var(--s-6); margin-bottom: var(--s-10); }
    .changelog-header .logo { height: 32px; width: auto; margin-bottom: var(--s-6); display: block; }
    .changelog-list { list-style: none; margin: 0; padding: 0; max-width: var(--measure); }
    .changelog-item { display: flex; flex-wrap: wrap; gap: var(--s-1) var(--s-3); padding: var(--s-3) 0; border-top: var(--bw-hair) solid var(--paper-edge); font-size: var(--fs-14); }
    .changelog-item:first-child { border-top: none; }
    .changelog-date { flex: none; color: var(--ink-3); font-family: var(--font-sans); font-size: var(--fs-12); }
    .changelog-detail { color: var(--ink-2); }
    .changelog-empty { color: var(--ink-2); }
    .changelog-footer { display: flex; align-items: center; gap: var(--s-4); flex-wrap: wrap; padding-top: var(--s-8); margin-top: var(--s-16); border-top: var(--border); }
    .changelog-footer .logo { height: 26px; width: auto; }
  </style>
</head>
<body>
  <noscript>
  ...
  <div class="changelog-wrap">
    <a class="changelog-back" href="/">&larr; Free Stack</a>
    <header class="panel changelog-header">
      <img class="logo" src="design-system/assets/kaipability-logo-lockup.png" alt="Kaipability">
      <p class="eyebrow">Free Stack</p>
      <h1>Changelog</h1>
      <p class="t-lede">What has been added, retired or repriced in the directory.</p>
    </header>
${entries.length
...
/* --- sitemap.xml (PRD section 18, amended by section 16's Phase 15 and
   Phase 16 clauses) -----------------------------------------------------
   Lists /, /faq.html, /privacy.html, /contact.html, /changelog.html (since
   Phase 16) and (once published) /how-we-choose.html. docs/how-we-choose.md
   exists but has not received Rocky's copy sign-off (BUILD-PLAN 14.3's
   conditional clause), so it ships without a sitemap entry until that
   sign-off lands; see TODO.md for the tracked gap. privacy.html and
   contact.html are hand-written static pages (BUILD-PLAN 15.1), not
   generated by this script; only their sitemap listing lives here.
   changelog.html IS generated by this script (the function just above),
   the same as index.html and faq.html. */
...
- Changelog (additions, retirements and price changes): ${SITE_URL}/changelog.html
...
function main() {
  ...
  // Not this generator's own output (scripts/changelog.mjs derives it from
  // tools.json's own git history, run on its own schedule); read only, the
  // same trust level presets.json gets just above.
  const changelogEntries = JSON.parse(readFileSync(join(ROOT, 'data', 'changelog.json'), 'utf8'));
  ...
  writeFileSync(join(ROOT, 'changelog.html'), buildChangelogHtml(changelogEntries));
  ...
  console.log('build-seo: wrote index.html, faq.html, changelog.html, sitemap.xml, llms.txt, robots.txt, data/faq.json');
}
