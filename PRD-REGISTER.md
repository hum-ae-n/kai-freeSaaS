...
`{ schemaVersion: 1, business: string, people: [string], accounts: [row], createdAt, updatedAt, revision: integer }`, plus the additive optional fields later phases recorded: `leavers` (Wave C) and `settings` (Phase 23: a plain object holding the Costs screen's user-set indicative rates `usdToGbp`/`eurToGbp`; the store passes it through with shallow validation and neither knows nor cares what callers keep there). `revision` increments on every save and drives conflict refusal (§6). Sovereign template rows (bank, HMRC Government Gateway, domain registrar, Meta Business, phone contract) are offered at setup as unticked suggestions, never auto-created.
...
- **9.4 Costs**: a ledger, not a dashboard: "next 14 days" and "next 60 days" renewal lists (date, service, amount), one monthly/annual toggle on a single total, and the existing indicative cost-growth chart beneath. No donuts.
  ...
  - **Money renders to two decimal places on this screen (Phase 23, Rocky's 14 Aug direction).** Every figure on the Costs screen (totals, subtotals, renewal amounts, exposure line, per-row notes, drawer) renders as pounds and pence: £54.60, never £54.6 and never £54. This is a ledger; ledgers carry pence. The public directory's whole-pound convention (~£200/yr value equivalents) is unchanged, since those are estimates, and estimates rendered to the penny would claim a precision they do not have.
  - **Spend-by-category bars (Phase 23).** Each category subtotal line carries a flat horizontal bar proportional to that category's share of the recorded total, industrial-neutral fill from the design system, rendered with plain CSS width, no chart library, no animation, no donuts (the ledger-not-dashboard rule stands; the numbers stay the primary reading and the bars are reinforcement, so a reader who cannot see them loses nothing).
  - **Billing currency (Phase 23).** Vendors bill small businesses in dollars and euros, and the gap between the sticker price and the card statement is part of the trap. Account rows gain an optional `currency` field, enum `GBP` (default), `USD`, `EUR`; the workspace document's settings gain optional user-set indicative rates (`usdToGbp`, `eurToGbp`: plain numbers the business chooses, editable from the Costs screen). A non-GBP costed row displays its native amount beside the converted figure ("$10.00, about £7.90 at your rate") and converts at the user's rate into every total and subtotal. With no rate set, non-GBP rows are excluded from GBP figures and counted plainly ("N rows billed in other currencies, no rate set"), never silently converted at a made-up rate. A quiet note states that conversions use the business's own indicative rate and that card issuers typically add 2 to 3 percent on foreign-currency billing. Live exchange rates are deliberately impossible here: the /my CSP is `connect-src 'self'` and the privacy promise is that nothing leaves the device, so no rate API may ever be called; the user's own recorded rate is a fact about their planning, which is exactly what the register stores. Both fields are additive and optional; `schemaVersion` stays 1 (the `planned` precedent); exports carry them, since the billed currency is a fact and the rate is the business's own recorded assumption.
