...
| `from` | **On `/my` only:** comma-separated tool ids (a `t:` prefix is also accepted) that pre-fill the account register with one row per tool, service and address already filled in. Built automatically by the client page's own "Set up your workspace" link (the whole stack) and by the Discover deck's hand-off (its "want to try" list). A raw value over 512 characters is ignored. |
| `have` | **On `/my` only, alongside `from`, Discover-only:** tool ids the visitor already uses, pre-filled as active accounts. Its mere presence, even as `?have=` with nothing after it, marks the arrival as a Discover deck hand-off, which is what makes the `from` list default to **planned** (intended, not signed up yet) rather than active. Nobody types either parameter by hand; both come from the "Open these in My Stack" link the deck itself builds. |
...
Below 768px, the public homepage leads with a short, swipeable deck of individual tools rather than dropping a first-time visitor straight into the full list; at wider widths it opens on demand from a "Discover" entry path above the list. Left, or the "Got it" button, means "I already use this"; right, or "Add to my list", means "I want to try it"; "Skip" is a button only, with no gesture, so an accidental drag can never discard a card by mistake. A deck never runs longer than 10 to 12 cards and always ends on a summary. Buttons and the keyboard (arrow keys to judge, Backspace to undo, Escape to close) work everywhere; the gesture is just a quicker way to do the same thing on a touch or mouse device.

Judgements are saved in the visitor's own browser only (`localStorage`, the same "saved on this device" honesty as the client-mode adoption tracker above): nothing is sent anywhere, and none of it survives a cleared cache or a switch to a different browser or device. A judged tool also shows a small "Got it" or "On my list" chip on that same tool's card in the ordinary browse list below, with a chooser to change or clear it, so the deck and the list can never disagree; on a mouse-and-hover device, an unjudged card in the list also grows two quick-judge corner buttons, letting a desktop visitor skip the deck altogether if they would rather work from the list.

The deck's completion screen offers "Open these in My Stack", which hands both lists to the workspace as a link (see the `from` and `have` rows in "URL schema" above): tools marked "want to try" arrive there as **planned** accounts, tools marked "got it" arrive as active ones. That screen stays live for as long as it's open, so judging a tool from the browse list's chips while it's showing updates its counts and its link too.
